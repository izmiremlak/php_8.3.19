...
<div class="content">
    <div class="container">
        <div class="row">
            <div class="col-sm-12">
                <h4 class="pull-left page-title">Video Galeri</h4>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="btn-toolbar" role="toolbar">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=video_galeri_ekle';"> <i class="fa fa-plus"></i> Yeni Video Ekle</button>
                                </div>
                                <div class="btn-group">
                                    <button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=video_kategoriler';"> <i class="fa fa-cog"></i> Video Kategorileri</button>
                                </div>
                                <div class="btn-group">
                    ...
                    <div class="panel panel-default m-t-20">
                        <div class="panel-body">
                            ...
                            if ($hesap->tipi != 2) {
                                ...
                                if ($sil !== null) {
                                    $db->prepare("DELETE FROM video_galeri WHERE id = ?")->execute([$sil]);
                                ...
                                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                    ...
                                    if (count($idler) > 0) {
                                        foreach ($idler as $id) {
                                            ...
                                            if ($action === 'sil') {
                                                $db->prepare("DELETE FROM video_galeri WHERE id = ?")->execute([$id]);
                                            }
                                        }
                                    }
                            ...
                            <table class="table table-hover mails datatable">
                                ...
                                <tbody>
                                    <?php
                                    $sorgu = $db->prepare("SELECT * FROM video_galeri WHERE dil = ? ORDER BY id DESC LIMIT 0, 500");
                                    $sorgu->execute([$dil]);
                                        ...
                                        <td>
                                            <div class="btn-group dropdown">
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    <li><a href="index.php?p=video_galeri_duzenle&id=<?= sanitizeInput((string)$row->id); ?>">Düzenle</a></li>
                                                    <li><a href="index.php?p=video_galeri&sil=<?= sanitizeInput((string)$row->id); ?>">Sil</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
