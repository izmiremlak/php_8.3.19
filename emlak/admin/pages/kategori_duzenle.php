            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <form role="form" class="form-horizontal" id="forms" method="POST" action="ajax.php?p=kategori_duzenle&id=<?= sanitizeInput($snc->id); ?>" onsubmit="return false;" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label for="ustu" class="col-sm-3 control-label">Ana Kategori</label>
                                <div class="col-sm-9">
                                    <select class="form-control" name="ustu" id="ustu">
                                        <option value="">Yok</option>
                                        <?php
                                        $kats = $db->query("SELECT * FROM kategoriler_501 WHERE tipi=0 AND ustu=0 AND dil='".$dil."' ORDER BY sira ASC");
                                        while ($kat = $kats->fetch()) {
                                        ?>
                                        <option value="<?= sanitizeInput($kat->id); ?>" <?= ($kat->id == $snc->ustu) ? 'selected' : ''; ?>><?= sanitizeInput($kat->baslik); ?></option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <p style="margin-left:10px;font-size:13px;margin-top:5px;">Yükleyeceğiniz görselin boyutları <?= sanitizeInput($gorsel_boyutlari['kategoriler']['resim2']['o']); ?> olmalıdır.</p>
                                </div>
                            </div>
                            <?= $fonk->bilgi(" Google'da rekabeti düşük kelimelerde organik olarak ilk sayfaya yükselmek için mutlaka aşağıdaki bilgileri doldurunuz. Sadece sayfa içeriği ile uyumlu anahtar kelimeler kullanınız."); ?>
                            <div class="form-group">
