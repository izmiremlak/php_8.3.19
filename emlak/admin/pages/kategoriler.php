...
if ($sil !== null && $hesap->tipi != 2) {
    $stmt = $db->prepare("DELETE FROM kategoriler_501 WHERE id = :id");
    $stmt->execute(['id' => $sil]);
    header("Location:index.php?p=kategoriler");
    exit;
}
...
<div class="content">
    <div class="container">
        <div class="row">
            <div class="col-sm-12">
                <h4 class="pull-left page-title">Kategoriler</h4>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="btn-toolbar" role="toolbar">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=kategori_ekle';"> <i class="fa fa-plus"></i> Yeni Kategori Ekle</button>
                                </div>
                                <div class="btn-group">
                    ...
                    <div class="panel panel-default m-t-20">
                        <div class="panel-body">
                            <table class="table table-hover mails datatable">
                                ...
                                <tbody>
                                    <?php
                                    $sorgu = $db->query("SELECT * FROM kategoriler_501 WHERE tipi=0 AND dil='".$dil."' ORDER BY id DESC LIMIT 0,500");
                                    while ($row = $sorgu->fetch()) {
                                        $ustu_baslik = 'Yok';
                                        if ($row->ustu != 0) {
                                            $ustu = $db->prepare("SELECT baslik FROM kategoriler_501 WHERE id = :id");
                                            $ustu->execute(['id' => $row->ustu]);
                                            $ustu_baslik = $ustu->fetchColumn() ?: 'Yok';
                                        }
                                    ?>
                                    <tr>
                                        ...
                                        <td><?= $dayarlar->permalink === 'Evet' ? 'kategori/' . sanitizeInput($row->url) : 'index.php?p=kategori&id=' . sanitizeInput((string)$row->id); ?></td>
                                        <td><?= sanitizeInput((string)$row->sira); ?></td>
                                        <td><?= sanitizeInput($row->tarih); ?></td>
                                        <td><?= sanitizeInput($ustu_baslik); ?></td>
                                        <td>
                                            <div class="btn-group dropdown">
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    <li><a href="index.php?p=kategori_duzenle&id=<?= sanitizeInput((string)$row->id); ?>">Düzenle</a></li>
                                                    <li><a href="index.php?p=kategoriler&sil=<?= sanitizeInput((string)$row->id); ?>">Sil</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
