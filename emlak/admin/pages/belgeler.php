...
if ($sil != "") {
    $db->query("DELETE FROM belgeler WHERE id=" . $sil);
...
if ($_POST) {
    ...
    if (count($idler) > 0) {
        foreach ($idler as $id) {
            $id = $gvn->rakam($id);
            if ($action == 'sil') {
                $db->query("DELETE FROM belgeler WHERE id=" . $id);
            }
        }
    }
    header("Location:index.php?p=belgeler");
...
<body>
    <div class="content">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">Belgeler</h4>
                </div>
            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="btn-toolbar" role="toolbar">
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=belge_ekle';"> <i class="fa fa-plus"></i> Yeni Belge Ekle</button>
                                    </div>
                                    <div class="btn-group">
                        ...
                        <div class="panel panel-default m-t-20">
                            <div class="panel-body">
                                <table class="table table-hover mails datatable">
                                    ...
                                    <tbody>
                                        <?php
                                        $sorgu = $db->query("SELECT * FROM belgeler WHERE dil='" . $dil . "' ORDER BY id DESC LIMIT 0,500");
                                            ...
                                            <td>
                                                <div class="btn-group dropdown">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li><a href="index.php?p=belge_duzenle&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">Düzenle</a></li>
                                                        <li><a href="index.php?p=belgeler&sil=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">Sil</a></li>
                                                    </ul>
                                                </div>
                                            </td>
                                        </tr>
