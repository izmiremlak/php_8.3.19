...
if ($sil != "") {
    $db->query("DELETE FROM subeler_bayiler_501 WHERE id=" . $sil);
...
if ($_POST) {
    ...
    if (count($idler) > 0) {
        foreach ($idler as $id) {
            $id = $gvn->rakam($id);
            if ($action == 'sil') {
                $db->query("DELETE FROM subeler_bayiler_501 WHERE id=" . $id);
            }
        }
    }
    header("Location:index.php?p=bayiler");
...
<body>
    <div class="content">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">Bayiler</h4>
                </div>
            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="btn-toolbar" role="toolbar">
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=bayi_ekle';"> <i class="fa fa-plus"></i> Yeni Bayi Ekle</button>
                                    </div>
                                    <div class="btn-group">
                        ...
                        <div class="panel panel-default m-t-20">
                            <div class="panel-body">
                                <table class="table table-hover mails datatable">
                                    ...
                                    <tbody>
                                        <?php
                                        $sorgu = $db->query("SELECT * FROM subeler_bayiler_501 WHERE turu=1 AND dil='" . $dil . "' ORDER BY id DESC LIMIT 0,500");
                                        ...
                                        <tr>
                                            ...
                                            <td><?= htmlspecialchars($row->lokasyon, ENT_QUOTES, 'UTF-8'); ?></td>
                                            ...
                                            <td>
                                                <div class="btn-group dropdown">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li><a href="index.php?p=bayi_duzenle&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">Düzenle</a></li>
                                                        <li><a href="index.php?p=bayiler&sil=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">Sil</a></li>
                                                    </ul>
                                                </div>
                                            </td>
                                        </tr>
