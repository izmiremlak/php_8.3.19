<?php
// Kullanıcının giriş yapıp yapmadığını ve doğru tipte olup olmadığını kontrol et
if ($_POST) {
    if ($hesap->id != "" AND $hesap->tipi != 0) {

        // ID ve slider bilgilerini kontrol et
        $id = $gvn->rakam($_GET["id"]);
        $snc = $db->prepare("SELECT * FROM slider_501 WHERE id=:ids");
        $snc->execute(['ids' => $id]);

        if ($snc->rowCount() > 0) {
            $snc = $snc->fetch(PDO::FETCH_OBJ);
        } else {
            die();
        }

        // POST verilerini temizle
        $baslik = $gvn->html_temizle($_POST["baslik"]);
        $sira = $gvn->zrakam($_POST["sira"]);
        $aciklama = $gvn->html_temizle($_POST["aciklama"]);
        $link = $gvn->html_temizle($_POST["link"]);

        // Resim yükleme işlemi
        $resim1tmp = $_FILES['resim']["tmp_name"];
        $resim1nm = $_FILES['resim']["name"];

        if ($resim1tmp != "") {
            $randnm = strtolower(substr(md5(uniqid(rand())), 0, 10)) . $fonk->uzanti($resim1nm);
            $resim = $fonk->resim_yukle(true, 'resim', $randnm, '../uploads', $gorsel_boyutlari['slider']['thumb_x'], $gorsel_boyutlari['slider']['thumb_y']);
            $resim = $fonk->resim_yukle(false, 'resim', $randnm, '../uploads', $gorsel_boyutlari['slider']['orjin_x'], $gorsel_boyutlari['slider']['orjin_y']);

            // Resim güncelleme işlemi
            if ($resim) {
                try {
                    $avgn = $db->prepare("UPDATE slider_501 SET resim=:image WHERE id=:id");
                    $avgn->execute(['image' => $resim, 'id' => $snc->id]);
                    if ($avgn) {
                        $fonk->ajax_tamam('Resim Güncellendi');
                        ?><script type="text/javascript">
                        $(document).ready(function(){
                            $('#resim_src').attr("src","../uploads/thumb/<?=$resim;?>");
                        });
                        </script><?php
                    }
                } catch (PDOException $e) {
                    error_log($e->getMessage(), 3, '/var/log/php_errors.log');
                    $fonk->ajax_hata('Resim Güncellenemedi. Bir hata oluştu: ' . htmlspecialchars($e->getMessage()));
                }
            } else {
                $fonk->ajax_hata('Resim Güncellenemedi. Bir hata oluştu!');
            }
        }

        // Slider bilgilerini güncelleme
        try {
            $updt = $db->prepare("UPDATE slider_501 SET baslik=:baslik, sira=:sira, aciklama=:aciklamax, link=:linkx WHERE id=:ids");
            $updt->execute([
                'baslik' => $baslik,
                'sira' => $sira,
                'aciklamax' => $aciklama,
                'linkx' => $link,
                'ids' => $snc->id
            ]);

            if ($updt) {
                $fonk->ajax_tamam("Slayt Güncellendi.");
            }
        } catch (PDOException $e) {
            error_log($e->getMessage(), 3, '/var/log/php_errors.log');
            $fonk->ajax_hata("Slayt Güncellenemedi. Bir hata oluştu: " . htmlspecialchars($e->getMessage()));
        }
    }
}