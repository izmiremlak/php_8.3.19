...
if ($_POST) {
    ...
    if ($hesap->id != "" && $hesap->tipi != 0) {
        ...
        try {
            // Üyelik paketi bilgilerini ekle
            $query = $db->prepare("INSERT INTO uyelik_paketleri_501 SET baslik=?, sira=?, renk=?, gizle=?, aylik_ilan_limit=?, ilan_resim_limit=?, ilan_yayin_sure=?, ilan_yayin_periyod=?, danisman_limit=?, danisman_onecikar=?, danisman_onecikar_sure=?, danisman_onecikar_periyod=?, ucretler=?");
            $query->execute([$baslik, $sira, $renk, $gizle, $aylik_ilan_limit, $ilan_resim_limit, $ilan_yayin_sure, $ilan_yayin_periyod, $danisman_limit, $danisman_onecikar, $danisman_onecikar_sure, $danisman_onecikar_periyod, $ucretler]);
        } catch (PDOException $e) {
            // Hataları log dosyasına kaydet ve sitede göster
            error_log("Üyelik paketi oluşturulamadı: " . $e->getMessage() . ". Tarih: " . date("Y-m-d H:i:s"));
            die($e->getMessage());
        }
