...
<div class="headerbg" <?= ($gayarlar->belgeler_resim != '') ? 'style="background-image: url(uploads/' . htmlspecialchars($gayarlar->belgeler_resim, ENT_QUOTES, 'UTF-8') . ');"' : ''; ?>>
    <div id="wrapper">
        <div class="headtitle">
            <h1><?= htmlspecialchars(dil("TX230"), ENT_QUOTES, 'UTF-8'); ?></h1>
            <div class="sayfayolu">
                <span><?= htmlspecialchars(dil("TX231"), ENT_QUOTES, 'UTF-8'); ?></span>
            </div>
        </div>
    </div>
...
<div class="uyeolgirisyap">
    <h4 class="uyepaneltitle"><?= htmlspecialchars(dil("TX230"), ENT_QUOTES, 'UTF-8'); ?></h4>

    <?php

    if ($hesap->turu == 1) {
        $dids = $db->query("SELECT kid,id,GROUP_CONCAT(id SEPARATOR ',') AS danismanlar_501 FROM hesaplar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND kid=" . (int)$hesap->id)->fetch(PDO::FETCH_OBJ);
    ...
    $qry = $pagent->sql_query("SELECT DISTINCT t1.id,t1.url,t1.tarih,t1.baslik,t1.durum,t1.ilan_no,t1.hit,t1.resim FROM sayfalar AS t1 LEFT JOIN dopingler_501 AS t2 ON t2.ilan_id=t1.id AND t2.ilan_tur=1 WHERE (t1.site_id_555=501 OR t1.site_id_888=100 OR t1.site_id_777=501501 OR t1.site_id_699=200 OR t1.site_id_701=501501 OR t1.site_id_702=300) AND t1.dil='tr' AND t1.ekleme=1 AND (t1.durum=1 OR t1.durum=0) AND t1.acid IN (" . $acids . ") AND t1.tipi=1 ORDER BY t1.tarih DESC");
    ...
    <tr id="row_<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">
        ...
        <span class="ilantarih"><?php if ($row->durum == 1 OR $row->durum == 3) { ?><?= htmlspecialchars(dil("TX237"), ENT_QUOTES, 'UTF-8'); ?> <?= htmlspecialchars($row->hit, ENT_QUOTES, 'UTF-8'); ?><?php } ?></span><br>
        <span class="ilantarih"><?= htmlspecialchars(dil("TX140"), ENT_QUOTES, 'UTF-8'); ?>: <?= htmlspecialchars($row->ilan_no, ENT_QUOTES, 'UTF-8'); ?></span>
        </td>
        <td align="center" id="mobtd">
        <?php
        if ($row->durum == 0) {
        ?><span style="color:red;font-weight:bold;"><?= htmlspecialchars(dil("TX238"), ENT_QUOTES, 'UTF-8'); ?></span><?php
        } elseif ($row->durum == 1) {
        ?><span style="color:green;font-weight:bold;"><?= htmlspecialchars(dil("TX239"), ENT_QUOTES, 'UTF-8'); ?></span><?php
        } elseif ($row->durum == 2) {
        ?><span style="color:green;font-weight:bold;"><?= htmlspecialchars(dil("TX240"), ENT_QUOTES, 'UTF-8'); ?></span><?php
        } elseif ($row->durum == 3) {
        ...
        <a title="Düzenle" class="uyeilankontrolbtn" href="uye-paneli?rd=ilan_duzenle&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
        <?php if ($row->durum == 1) { ?>
        <a title="Pasif Yap" class="uyeilankontrolbtn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_pasif&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>','hidden_result');"><i class="fa fa-eye-slash" aria-hidden="true"></i></a>
        <?php } ?>
        <a title="Sil" class="uyeilankontrolbtn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_sil&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>','hidden_result');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
        </td>
    </tr>
    <?php
    }
    ...
    ?> 
    <h4 style="text-align:center;margin-top:60px;"><?= htmlspecialchars(dil("TX242"), ENT_QUOTES, 'UTF-8'); ?></h4>
    <?php
    }
    ?>
