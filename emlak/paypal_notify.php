...
// POST isteği gelmemişse işlemi sonlandırır.
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    error_log("Ödeme Gerçekleşemedi: POST dönmedi", 3, "/var/log/php_errors.log");
    die("Ödeme Gerçekleşemedi.");
}

// Giriş verilerini temizler.
$secret = $gvn->html_temizle($_GET["secret"]);
$tutar = $gvn->html_temizle($_POST["mc_gross"]);
$kesinti = $gvn->html_temizle($_POST["mc_free"]);
$item_tutar = $tutar - $kesinti;
$email = $gvn->html_temizle($_POST["payer_email"]);
$item_id = $gvn->html_temizle($_POST["item_number"]);
$customs = $gvn->html_temizle($_POST["custom"]);

// Secret kontrolü.
if ($secret != PAY_SECRET) {
    error_log("Ödeme Gerçekleşemedi: Geçersiz Token", 3, "/var/log/php_errors.log");
    die("Ödeme Gerçekleşemedi.");
}
...
if ($satis == "doping_ekle") {
    ...
    $odeme_yontemi = "Kredi Kartı (PAYPAL)";
    $tarih = $fonk->datetime();
    $durum = 1;
    $hesap_id = $hesapp->id;
